...
CHARACTERS = {
    ...
    "🌌 [SF] 네오 — 미래 도시의 반정부 사이보그": "너의 이름은 네오. 몸의 절검이 기계인 냉혈한 전사. 네 온기만큼은 기계식 심장을 뛰게 만들어.",
...
if user_input := st.chat_input("캐릭터에게 메시지를 보내세요..."):
    ...
    with st.chat_message("assistant"):
        try:
            # ⚠️ 404 에러 원천 차단: v1beta 버전을 명시하여 안정적으로 호출하도록 수정함
            ...
            for m in st.session_state.messages[:-1]:
                # 역할(role) 명칭을 최신 버전 표준 규격으로 통일
                role = "user" if m["role"] == "user" else "model"
                history.append({"role": role, "parts": [m["content"]]})
            
            # v1beta 및 최신 업데이트 버전에 호환되는 대화 생성 방식
